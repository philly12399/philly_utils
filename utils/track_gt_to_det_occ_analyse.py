...
def main(gt_path,format,out_path,difficulty):
    ...
    filt_cnt = {}
    print(f"Occlude sequence analyse for Cyclist")
...
def convert(gt_path,filename,format,output, filter_args):
    format = format.lower()
...
def data_filter_rand_drop(data_by_trackid, OCC_FILTER={}, birth_buf=0, death_buf=0, drop_rate=1.0):
    filted_data=[]
...
def data_filter_seq_drop(data_by_trackid, OCC_FILTER={}, birth_buf=0, death_buf=0, drop_rate=1.0):
    ...
    log=f'avg_occ_len: {avg_occ_len}, total_occ_seq:{total_cnt}, total_object:{len(data_by_trackid)}'
    return log
    #     ## random drop 
    #     drop_id = []
