...
def test(track_path='./output_bytrackid/car_mark_all_rotxy'):
    ...
    for i, tid in enumerate(track):
        ...
        for fi, f in enumerate(frames):
            ...
            voxel = voxelize(pcd,bbox,0.5)
            drawbox(vis,bbox,color = [1,0,0])
            p_mean = []
            pts = []
            
            # vv= [voxel[vi] for vi in [0, 1,40,5] ]
            for v in voxel:
                # drawbox(vis,v)
                if(len(v["pts"]) > 0):
                    pts += v["pts"].tolist()
...
def voxelize(pcd, box, voxel_size=0.3, overlap=True):
    ...
    #regular
    for p in pcd:
        i,j,k = int((p[0]+l/2)/(voxel_size/2)), int((p[1]+w/2)/(voxel_size/2)), int((p[2]+h/2)/(voxel_size/2))    
        for di in neighbor:
            for dj in neighbor:
                for dk in neighbor:                        
